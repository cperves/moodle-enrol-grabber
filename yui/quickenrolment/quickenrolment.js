YUI.add('moodle-enrol_grabber-quickenrolment', function(Y) {
    ...
    Y.extend(USERENROLLER, Y.Base, {
        ...
        initializer : function(config) {
                ...
                .append(create('<div class="'+CSS.WRAP+'"></div>')
                    ...
                    .append(create('<div class="'+CSS.CONTENT+'"></div>')
                        .append(create('<div class="'+CSS.SEARCHCONTROLS+'"></div>')
                            .append(create('<div class="'+CSS.ENROLMENTOPTION+' '+CSS.ROLE+'"><label for="id_enrol_grabber_assignable_roles">'+M.util.get_string('assignroles', 'role')+'</label></div>')
                                    .append(create('<select id="id_enrol_grabber_assignable_roles"><option value="">'+M.util.get_string('none', 'enrol')+'</option></select>'))
                            )
            ...
            Y.all('.enrol_grabber_plugin input').each(function(node){
            ...
            if (this.get(UEP.COHORTSAVAILABLE)) {
                this.get(UEP.BASE).one('.'+CSS.ENTITYSELECTOR)
                    .append(create('<input type="radio" id="id_enrol_grabber_entity_users" name="enrol_grabber_entity" value="users" checked="checked"/>'))
                    .append(create('<label for="id_enrol_grabber_entity_users">'+ M.util.get_string('browseusers', 'enrol_grabber')+'</label>'))
                    .append(create('<input type="radio" id="id_enrol_grabber_entity_cohorts" name="enrol_grabber_entity" value="cohorts"/>'))
                    .append(create('<label for="id_enrol_grabber_entity_cohorts">'+M.util.get_string('browsecohorts', 'enrol_grabber')+'</label>'));
                this.get(UEP.BASE).one('#id_enrol_grabber_entity_cohorts').on('change', this.search, this);
                this.get(UEP.BASE).one('#id_enrol_grabber_entity_users').on('change', this.search, this);
            } else {
                this.get(UEP.BASE).one('.'+CSS.ENTITYSELECTOR)
                    .append(create('<input type="hidden" name="enrol_grabber_entity" value="users"/>'));
            }
        ...
        show : function(e) {
            ...
            var rolesselect = Y.one('#id_enrol_grabber_assignable_roles');
            if (rolesselect) {
                rolesselect.focus();
            }
        },
        ...
        currentEntity : function() {
            var entity = CSS.USER;
            var cohortsinput = Y.one('#id_enrol_grabber_entity_cohorts');
    ...
    M.enrol_grabber = M.enrol_grabber || {};
    M.enrol_grabber.quickenrolment = {
        init : function(cfg) {
